...
int main() {
    ...
    float *stdDev = (float *)malloc(IMAGE_WIDTH * IMAGE_HEIGHT * sizeof(float));

    const char *programSource =
        "__kernel void convertToUInt16(__global uchar *imageData, __global ushort *outputData) {"
        "    int gid = get_global_id(0);"
        "    outputData[gid] = (ushort)imageData[gid];"
        "}"
        "__kernel void calculate_stats(__global ushort *inputImage, "
        "__global int *sum, "
        "__global float *mean, "
    ...
    save_png("converted_image.png", imageData, IMAGE_WIDTH, IMAGE_HEIGHT);
    ...
    cl_mem stdDevBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * IMAGE_WIDTH * IMAGE_HEIGHT, NULL, NULL);
    ...
    clSetKernelArg(statsKernel, 3, sizeof(cl_mem), &stdDevBuffer);
    ...
    clEnqueueReadBuffer(queue, stdDevBuffer, CL_TRUE, 0, sizeof(float) * IMAGE_WIDTH * IMAGE_HEIGHT, stdDev, 0, NULL, NULL);
