...
#define PADDING 4
#define PADDED_WIDTH (IMAGE_WIDTH + 2 * PADDING)
#define PADDED_HEIGHT (IMAGE_HEIGHT + 2 * PADDING)
uint16_t paddedImage[PADDED_HEIGHT * PADDED_WIDTH];
...
int main() {
    ...
    for (int i = 0; i < PADDED_HEIGHT; i++) {
        for (int j = 0; j < PADDED_WIDTH; j++) {
            paddedImage[i * PADDED_WIDTH + j] = 0;
        }
    }
  ...
  float rangeinv = 1 / range;
    ...
    const char *programSource =
        ...
        "            }"
        "    else"
        " // Handle zero-padding for out-of-bound indices"
        "        windowStdDev += pow(-windowMean, 2);"
        "        }"
        "    }"
        "  }"
    ...
    clEnqueueWriteBuffer(queue, inputBuffer, CL_TRUE, 0, sizeof(uint16_t) * PADDED_WIDTH * PADDED_HEIGHT, paddedImage, 0, NULL, NULL);
    // Load OpenCL code
...
for (int i = 0; i < IMAGE_HEIGHT; i++) {
 ...
 // Copy the original image to the center of the zero-padded image
    for (int i = 0; i < IMAGE_HEIGHT; i++) {
        for (int j = 0; j < IMAGE_WIDTH; j++) {
            paddedImage[(i + PADDING) * PADDED_WIDTH + (j + PADDING)] = image[i * IMAGE_WIDTH + j];
        }
    }
